...
| 400 | 手机号格式不正确 / 关系枚举非法 |
| 403 | 老人账号或陪诊员调用 |
| 2001 | 该手机号没有对应的老人账号；**也包括「老人已被其他家属绑定」**（见下方说明） |
| **409** | 该老人**已经绑定到你自己**名下（不重复建关系） |
| **501** | `bindType=INVITE_CODE` |

> ⚠️ **「已被其他家属绑定」返回的是 `2001` 而不是 `2002`，这是有意的。**
>
> 该手机号「不存在老人账号」「账号被封禁」「还没建档」「已被别人绑定」四种情况，
> 对外**统一**回同一条文案「未找到可绑定的老人账号」。原因是：如果「已被别人绑定」
> 单独回 `2002`，攻击者就能拿一批手机号去试探，靠 `2001 / 2002` 的差异
> **枚举出哪些号码对应已注册的老人账号** —— 这是典型的账号枚举侧信道。
>
> 代价是家属 B 想绑一位已被家属 A 绑定的老人时，看到的提示不够精确。
> 这是一个**有意的取舍**：隐私优先，精确引导交给线下客服。
>
> 因此 `ResultCode.ELDER_ALREADY_BOUND(2002)` 目前**不会被任何接口抛出**，
> 仅作为错误码槽位保留（前端可以照常处理，只是收不到）。
