...
| 登出 | 把当前请求携带的 `accessToken` 与 `refreshToken` 的 `jti` 写入 Redis 黑名单直至自然过期；**按设备登出**，该账号其它设备已签发的令牌不受影响 |

> ⚠️ **登出 = 按设备下线**：登出只拉黑当前设备那一对 `jti`，同一账号在手机 / 网页 / 桌面端并发登录时，其它设备手里的令牌**仍然有效**（HTTP 200）。
> 若需要「一键踢全部」（如账号被盗处置），走**修改密码**或 `AdminService#disableUser` —— **只有这两条路径**才递增密码版本、作废该账号全部令牌。

> **`ver` 必须等于 Redis 的 `pwd:version:{userId}`**，否则认证过滤器直接判为无效令牌（HTTP 401）。
> 该键**没有 TTL**：首次登录时由 `ensurePasswordVersion` 显式写入 `0`（防止「Redis 被清空后旧 `ver=0` 令牌复活」），**修改密码 / 管理员封禁**时 `+1`（登出**不**动它）。
...
### 2.5 参数校验与角色校验的时序（F-05 口径澄清）

`/api/admin/**` 以外的业务接口，角色判定走 Spring AOP 的 `@PreAuthorize`，
**晚于参数解析**。这与 `/api/admin/**` 在过滤链上做角色预检的策略不同，
原因与后果如下。

| 路径 | 角色判定时机 | 越权 + 非法体的响应 |
|---|---|---|
| `/api/admin/**` | `SecurityConfig` 过滤链（早于参数解析） | 一律 403，**看不到请求体的错误信息** |
| `/api/order/**`、`/api/execution/**`、`/api/medication/**` 等业务接口 | `@PreAuthorize` AOP（晚于参数解析） | **参数校验业务错误先返**（HTTP 200 + `code=400` `PARAM_ERROR`，如缺少必填请求体），角色不匹配要等合法体才看得到 403 |
| 同上的业务接口，但调用者是 **ELDER** | `ElderReadOnlyInterceptor`（`WebMvcConfig` 注册于 `/api/**`，同样早于参数解析） | **一律 403**，与请求体是否合法无关 |

**为什么这样设计**：

- `/api/admin/**` 越权者可能用「非法体探针」探测权限边界，必须在看不到请求体的前提下拒接，
  所以角色判定下沉到过滤链。
- 业务接口越权者是「错误角色但有合法意图」（如 COMPANION 想用 `/api/order` 下单），
...
- 现有 `security/*MatrixTest` 已遵守：`ExecutionAccessMatrixTest#jpeg()`
  与 `AdminAccessMatrixTest` 的 JSON body 都构造了合法字段。
...
1. **令牌必须用「实时密码版本」签发**，不要写死 `ver=0`。
   改密 / 管理员封禁会把 `pwd:version:{userId}` 递增且**永不过期**（登出**不会** —— F-01 起登出只按 `jti` 拉黑）；
   只要库里跑过一次改密 / 封禁，任何硬编码 `ver=0` 的用例都会拿到 **401（验票失败）而不是预期的 403**，现象极具迷惑性。
...
| v0.3.1 | 2026-09-15 | 修正 `2002` 说明（绑定冲突统一回 `2001` 防枚举，`2002` 为预留槽位）；补全 Token 载荷字段（`jti` / `ver`）与**登出会递增密码版本**这一行为（**已于 v0.3.2 变更**）；新增三条测试约定（实时密码版本 / lambda 缓存预热 / 端到端自清理） | — |
| v0.3.2 | 2026-09-19 | 登出语义由「按用户递增密码版本、全端下线」改为**按设备（`jti`）拉黑**（`AuthServiceImpl#logout`）；同步 §2.2 Token 规格、§2.5 参数 / 角色校验时序（补 ELDER 例外与正确错误码）、测试约定第 1 条与 `01-auth-user.md`；`pwd:version` 递增收敛为改密 / 管理员封禁 | — |
