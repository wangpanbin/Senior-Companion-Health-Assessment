...
| `/api/order/**`、`/api/execution/**`、`/api/medication/**` 等业务接口 | `@PreAuthorize` AOP（晚于参数解析） | **参数校验业务错误先返**（如 3001），角色不匹配要等合法体才看得到 403 |
...
**越权矩阵测试侧的要求**（参 {@code reports/playwright/e2e-report.md §F-05}）：

- 断言「角色不匹配 → 403」时，**必须用合法请求体**，否则会拿到参数校验业务错误而非 403。
- 现有 {@code security/*MatrixTest} 已遵守：{@code ExecutionAccessMatrixTest#jpeg()}
  与 {@code AdminAccessMatrixTest} 的 JSON body 都构造了合法字段。
