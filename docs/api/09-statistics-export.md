...
- **`granularity` 回显为小写**（`day` / `week` / `month`）：请求参数用枚举名 `DAY`/`WEEK`/`MONTH`，
  回显值给前端当图表标识用，小写更贴合 ECharts 侧的写法。两者不是同一个东西，**不要混用**。
- `categories` 的取值随粒度变化：`DAY` 为 `2026-09-15`；`WEEK` 为桶起始日 + `" 当周"`（如 `2026-09-14 当周`，
  周一为起）；`MONTH` 为 `2026-09`。三种粒度下**序列求和都不变**，分桶只改横轴。
...
- **图与汇总放同一个响应里**（`chart` + `summary`），不拆成两个接口：前端是一个「图 + 卡」的页面，
  拆开就有两次往返，而且两次查询之间数据可能已经变化 —— 图表与汇总卡数字对不上，
  是最容易被截图当成 bug 的那类问题。
- `summary` 的口径与 `chart` 严格自洽：`summary.taskTotalCount` = 「任务总数」序列求和，
  以此类推。改任一侧都必须同时改另一侧。
- 漏服口径为 `was_missed = 1`（**含漏服后补记**），与 `/api/medication/calendar` 一致。
  按 `status = 'MISSED'` 统计会让「列表里一条漏服都没有」而「漏服率不为 0」，数字与列表互相矛盾。
- `elderId` 只影响本接口，用于「这位老人的用药依从性如何」的单人视角。
