...
2. **登出用例必须用「专用账号 + 空登录态」**，不能挂在共享 `storageState` 上 ——
   登出会作废**当前会话**的令牌（`AuthServiceImpl#logout` 按 `jti` 拉黑当前 `accessToken` + `refreshToken`）。
   当前专用账号：`comp025`。
   > 历史注记：本规则在登出还是「按用户递增 `pwd:version`」时是**硬性**的 —— 一次登出会让该账号**全部**令牌失效，
   > 后续复用其登录态的 spec 全线 401。F-01 改为按 `jti` 拉黑后，这份风险已消失，
   > 但「专用账号 + 空登录态」仍**推荐**保留：它让登出用例既不依赖、也不影响任何其它 spec 的登录态。
