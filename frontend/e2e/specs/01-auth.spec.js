...
test.describe('注销', () => {
  // ⚠️ 两条硬约束（实测踩过）：
  //   1) 不复用共享 storageState —— 登出会作废该账号的令牌；
  //   2) 用**专用账号** comp025（其余 spec 不依赖它的登录态）。
  //   根因：后端的登出是**按用户**生效的（登出时自增 Redis `pwd:version:<userId>`），
  //   一次登出会作废该账号的**全部**令牌。若拿 fam001 做登出，后续所有
  //   复用 fam001 storageState 的页面巡检会全线 401。
