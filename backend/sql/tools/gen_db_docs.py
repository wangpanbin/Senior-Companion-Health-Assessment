...
def doc_er(tables, counts):
    ...
    L.append("    PENDING --> CANCELLED : 家属/管理员取消")
    L.append("    ACCEPTED --> IN_SERVICE : 陪诊员到院打卡")
    L.append("    ACCEPTED --> CANCELLED : 家属/管理员取消")
    L.append("    IN_SERVICE --> COMPLETED : 陪诊员完成并提交小结")
    L.append("    IN_SERVICE --> CANCELLED : 管理员强制终止")
    ...
    L.append("| 已接单 | `ACCEPTED` | 陪诊员到院打卡 / 取消 | `order_status_log` + `order_checkin` |")
    L.append("| 服务中 | `IN_SERVICE` | 节点打卡 / 上传轨迹 / 提交小结完成 | `order_status_log` + `order_checkin` + `companion_track` |")
    L.append("| 已完成 | `COMPLETED` | 家属评价 / 管理员核定结算 | `order_status_log` + `companion_profile` 聚合 |")
